  ...
  try {
    ...
    if(d.wrs?.length) {
      ...
      Object.entries(buckets).forEach(([name,items])=>{
        html += `<div><div class="card" style="padding:14px 16px;max-height:500px;display:flex;flex-direction:column"><h3 style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px;flex-shrink:0"><span>${icons[name]||''} ${name}</span><span style="font-size:10px;color:var(--dim);font-weight:700">${items.length}</span></h3><div style="overflow-y:auto;flex:1;scrollbar-width:thin">${items.map(w=>`<div class="wr-card"><div class="title" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${w.title}</div><div class="meta">${w.type} ${w.priority?`· ${w.priority}`:''}</div></div>`).join('')||'<div style="color:var(--dim);font-size:11px;text-align:center;padding:16px;border:1px dashed var(--border);border-radius:10px">Empty</div>'}</div></div></div>`;
      });
      html += `</div></div>`;
    }
    // Update badge
