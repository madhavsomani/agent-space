...
async function refreshSystem() {
  try {
    ...
    document.getElementById('sys-top').innerHTML=`
      <div class="card" style="padding:16px 18px"><h3 style="display:flex;align-items:center;justify-content:space-between;gap:8px">CPU ${sparklineSVG(cpuHistory,60,20,cpuPct>80?'#ef4444':cpuPct>50?'#f59e0b':'#22c55e')}</h3><div class="metric ${barColor(cpuPct)}">${cpuPct}%</div><div class="sub">User ${d.cpu.user}% · Sys ${d.cpu.sys}% · Idle ${d.cpu.idle}%</div><div class="bar-bg" style="margin-top:8px"><div class="bar-fill ${barColor(cpuPct)}" style="width:${cpuPct}%"></div></div></div>
      <div class="card" style="padding:16px 18px"><h3 style="display:flex;align-items:center;justify-content:space-between;gap:8px">Memory ${sparklineSVG(memHistory,60,20,memPct>80?'#ef4444':'#3b82f6')}</h3><div class="metric ${barColor(memPct)}">${d.memory.usedGB} GB</div><div class="sub">${memPct}% of ${d.memory.totalGB} GB</div><div class="bar-bg" style="margin-top:8px"><div class="bar-fill ${barColor(memPct)}" style="width:${memPct}%"></div></div></div>
      <div class="card" style="padding:16px 18px"><h3 style="margin-bottom:6px">Disk</h3><div class="metric ${barColor(diskPct)}">${d.disk.used}</div><div class="sub">${diskPct}% of ${d.disk.total}</div><div class="bar-bg" style="margin-top:8px"><div class="bar-fill ${barColor(diskPct)}" style="width:${diskPct}%"></div></div></div>`;
    _cachedSystem = d;
